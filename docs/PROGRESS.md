...
- **Inline media in text notes renders blank in Board cards + missing on Timeline + tap-to-fullscreen skips zoom transition.** Three-part issue, needs investigation per part:
    - **Board card blank.** A text note with an attached image renders an empty white box in `KeepCard`'s body. Tapping the white space opens `MediaViewerScreen` and the image **does** show fullscreen — so `payload.data` is set; the card-side render is what's broken. [InlineMediaBlockView.posterImage()](apps/ios/DailyCadence/DailyCadence/DesignSystem/Components/InlineMediaBlockView.swift) only checks `posterData` then `data`; **doesn't check `thumbnailData`**. Phase F.1.1b made cards prefer thumbnails but inline-block render didn't get updated. Fix: add `thumbnailData` to the lookup chain (prefer thumb → data → posterData). Verify the same logic for video where `posterData` is the right slot.
    - **Timeline missing entirely.** `MockNote.timelineMessage` flattens paragraph blocks to a single AttributedString and **intentionally** drops media — the design call was that the dense Timeline rail favors text-only summaries, with the Board view as the medium-rich surface. Per FEATURES.md line 264. Decision needed: is this still the intent, or do we want inline media on the Timeline too? If the latter, `NoteCard` needs to render media blocks too.
    - **No zoom transition.** [InlineMediaBlockView.swift:82](apps/ios/DailyCadence/DailyCadence/DesignSystem/Components/InlineMediaBlockView.swift) presents the viewer via `.fullScreenCover` with no source frame — falls back to the legacy slide-up presentation. Fix: route through `mediaTapHandler` like `KeepCard.mediaScaffold` does, but with a per-block source id (e.g., `note.id` combined with the block's id) so each inline image gets its own matched-geo source. Each block also needs a `MatchedGeometryModifier` to publish its frame via `CardFrameKey`.
- **Settings → Note Types row's "6 customized" trailing detail wraps to 2 lines.** Visible in Jon's screenshot at 2026-04-27. The Settings list cell template doesn't budget enough horizontal space for the detail label. **Fix options:** (a) shorten the label to "6 custom" or use a numeric-only count with an icon, (b) `.lineLimit(1)` + `.fixedSize(horizontal: true, ...)` on the detail label, (c) restructure the row so the detail sits on its own line when long. Path: [Features/Settings/SettingsScreen.swift:210](apps/ios/DailyCadence/DailyCadence/Features/Settings/SettingsScreen.swift:210).
- **Intro paragraph text duplicated below the inline image (regression of Phase E.5.18b).** Repro per Jon at 2026-04-27: typed multi-line text into the intro editor, attached an image; the trailer editor below the image came up populated with the **same text** (verbatim, multi-line) instead of an empty target. Phase E.5.18b shipped a fix + regression test (`insertMediaIntoSingleParagraphBodyAddsDistinctTrailingParagraph`) for what looks like the same bug — but the original repro was a single-line "my snack for today" + insert. Jon's repro is a multi-paragraph block with newlines that may take a different path through `NoteDraftStore.insertMedia`. **Investigation TODO:** (a) figure out the exact repro the test is missing, (b) check whether the bug fires on EDIT of an existing note with media (the test only covers fresh-draft insert), (c) check whether saving + reloading the note (round-trip through `NotesRepository`) produces the duplicate. Likely root: the leading/trailing paragraph identity check assumes single-paragraph bodies; with multi-paragraph or edit-mode bodies it can resolve to the same block, mirroring text. Add a regression test once the exact path is found.
...
- **Rename "Primary color" in Settings → Appearance.** Jon at 2026-04-27: the "Primary color" label feels engineering-flavored — wants something cuter or more theme-flavored. Brainstorm options: **"Theme color"** (clearest, matches industry convention), **"Vibe"** (playful, fits DailyCadence's softer voice), **"Cadence color"** (brand-tying — leverages the app name), **"Accent"** (Apple-ish minimal). Recommend **"Theme color"** as the safe pick or **"Vibe"** if leaning into the brand's friendly tone. The internal `ThemeStore` / `PrimaryPaletteRepository` names can stay — this is a UI label change in [SettingsScreen.swift](apps/ios/DailyCadence/DailyCadence/Features/Settings/SettingsScreen.swift) only.
