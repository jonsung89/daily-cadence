...
- ~~**Scroll-edge soft-fade (Apple Messages pattern).**~~ Shipped Phase F.1.2.scrolledge for TimelineScreen (the only screen with significant scrollable content right now). Other surfaces (Settings List, placeholder screens, editor sheets) can adopt the same modifier when they get real content — pattern is `.scrollEdgeEffectStyle(.soft, for: .top)` on the relevant scroll view.
