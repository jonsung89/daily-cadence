...
- **Scroll-edge soft-fade (Apple Messages pattern).** As content scrolls under the status bar / nav-bar area, fade the top edge so text doesn't crash through the system chrome. iOS 26's [`.scrollEdgeEffectStyle(.soft, for: .top)`](https://developer.apple.com/documentation/swiftui/scrollview) is the modern declarative path — added specifically for this pattern at WWDC25, used by Messages / Mail / Notes. Apply to the main scroll surfaces: TimelineScreen's note list (both Timeline and Board layouts), CalendarScreen, DashboardScreen, LibraryScreen, SettingsScreen. Verify the effect plays nicely with: existing `.safeAreaInset(.bottom)` TabBar, the FAB overlay, and the loading-bar overlay at the top of TimelineScreen (the loading bar should sit ABOVE the fade, not be subject to it). Fall back to `.scrollEdgeEffectStyle(.hard)` only where a visible separator is needed.
