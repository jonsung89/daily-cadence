...
**Last updated:** 2026-04-27 (Phase F.1.2.bugbash — six bug fixes in one round: intro-text duplicated below inline image (E.5.18b regression on the populate path), past-event note insert position, inline media renders blank in Board cards, inline media tap skips zoom, timeline time-label wraps, Settings "6 customized" wraps. +5 regression tests, 84/84 passing.)
...
- **FAB menu copy refresh** — Jon at 2026-04-27 wants the three FAB items renamed to feel less engineering-flavored and more in-brand-voice (DailyCadence is warm/friendly). Current copy: "Text Note" / "Photo or Video" / "Take Photo or Video". Brainstorm options below — pick one per row before implementing. **"Text Note" → broader** (covers any thought, not just text):
    - "Write a thought" (Jon's starting suggestion — verb-first, friendly)
    - "Jot something down"
    - "New entry"
    - "Quick thought"
    - "Reflect"
    
    **"Photo or Video" → library-flavored**:
    - "Add from Photos" (Jon's starting suggestion)
    - "Pick from library"
    - "From your photos"
    - "Browse Photos"
    
    **"Take Photo or Video" → camera-flavored** (avoid "Capture Moment" — competitor's wording):
    - "Snap something"
    - "Quick snap"
    - "Camera"
    - "Catch it"
    ...
    All three changes are pure copy edits in [TimelineScreen.swift](apps/ios/DailyCadence/DailyCadence/Features/Timeline/TimelineScreen.swift) Menu items. SF Symbols can stay or be reconsidered along with the copy.
...
- **Note type picker — scaling beyond ~7 types.** Per Jon at 2026-04-27: the editor's horizontal-scroll chip row is workable at 6–7 types but becomes a hunt as we add Pets, Book, Recipe, future custom user types. Decision friction at the start of a draft breaks the "free flow" the editor is supposed to support. Options to consider, in increasing reorganization:
