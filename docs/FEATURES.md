...
- **Swipe horizontally on the strip** → previous / next week. Same-weekday selection in the new week (Apple Calendar pattern: Wed → swipe → Wed of next week). Mirrors the timeline's day-swipe — `simultaneousGesture` + `DragGesture(minimumDistance: 30)` + horizontal-dominance guard (`|dx| > |dy| × 1.5`, `|dx| > 60`). Soft `UIImpactFeedbackGenerator` haptic on commit. Per-cell taps and long-presses still arbitrate cleanly because the drag's 30pt minimum lets quick taps fall through and the long-press's 50pt `maximumDistance` tolerates minor jitter.
