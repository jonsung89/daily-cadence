...
- **Tappable** — tap a column → `TimelineStore.shared.selectDate(...)`. Doubles as day-level navigation alongside the chevrons.
- **Apple Calendar–style dial** (Phase F.1.2.weekstrip.dial) — paged via `TabView(.page(indexDisplayMode: .never))`. Day-of-week letters (S M T W T F S) render in a fixed row above the TabView and **never move**; only the date numbers slide horizontally as the user drags. TabView gives the interactive drag, peek of adjacent weeks during the swipe, snap-on-release physics, and velocity-based commit — all native, no custom gesture math. Soft `UIImpactFeedbackGenerator` haptic when a swipe commits to a new week. **Selection sync:** the TabView's selection is the visible week's start date; when the user pages, `selectedDay` shifts by the matching day delta to preserve the weekday (Wed → Wed in the new week, Apple Calendar pattern). When `selectedDay` changes externally (chevron tap, day picker, etc.), the TabView animates to the matching week. Page range is ±52 weeks from app-launch (105 total) — TabView is lazy so off-screen pages don't pay layout costs. Per-cell taps and long-presses still arbitrate cleanly inside each page; TabView's pan gesture handles horizontal swipe physics, individual cells handle their own tap / long-press / hit-area pad.
- **Long-press → emoji marker** (Phase F.1.2.daymarks) — long-press (0.35 s, `maximumDistance: 50pt` so finger jitter during the hold doesn't silently cancel) any column to mark that day with an emoji. The cell scales to 0.94× during the hold (spring-back when the sheet takes over). Two haptics make the gesture feel native: a soft `UIImpactFeedbackGenerator(.soft)` on touch-down (the "I felt that" feedback iOS context menus give) and a medium one on long-press completion. Each column carries an extra 4pt-vertical / 2pt-horizontal hit-area pad outside its visible bounds so a slightly-off finger still registers without growing the strip's visible height.
- **`EmojiPickerSheet`** (Phase F.1.2.daymarks) — reusable bottom-sheet emoji picker, mirrors iOS-native reaction trays (Messenger / Discord / iMessage). `.medium` + `.large` detents with drag indicator, slide-up animation, dimmed backdrop, swipe-down to dismiss — all free with `.sheet`. Layout: subtitle + bold title (e.g., "Mark this day" + the weekday/month/day) → search field → "Commonly used" curated quick-picks (caller-supplied per feature) → "Recent" (last 10 picks for that feature, persisted to `UserDefaults` per `recentStorageKey`) → six categorized sections (Celebrations / Hearts / Faces & people / Food & drink / Travel & places / Symbols) with ~18 emojis each, sourced from `EmojiCatalog.default` (~108 hand-picked). Search filters by emoji-keyword (lowercase substring match against `EmojiCatalog.keywords`), category title, or the emoji itself. Designed for reuse: any future feature that needs emoji input (message reactions, mood tagging, etc.) passes its own `commonlyUsed` curation + `recentStorageKey` and gets the same UX.
- **Day-mark configuration:** the picker's `commonlyUsed` for week-strip day marks is 20 special-day picks (🎂 🎉 ❤️ 💍 ⭐ / ✨ 🎁 🎈 🍾 🥂 / 👶 🎓 🌈 ✈️ 🏠 / ❗ 📅 🏆 💐 🌙), `recentStorageKey` is `com.jonsung.DailyCadence.daymarks.recentEmojis`, and `onRemove` surfaces a wide Remove footer button (fg2 + trash icon) only when the day is already marked. Selection commits and dismisses; the chosen emoji renders in the cell's top-right corner (~14pt, offset 2pt up/right of the rounded-rectangle corner) and animates in via `.bouncy(duration: 0.4, extraBounce: 0.2)` scale-in spring; removes via fade. Marks sync across devices via the `day_marks` Supabase table — household TestFlight scenario: marking the anniversary on one phone surfaces on the partner's.
...
Day marks are backed by `DayMarkStore` (`@Observable @MainActor` singleton) keyed by `startOfDay`; bulk-fetched once via `DayMarkRepository.fetchAll(userId:)` in `RootView.task(id:)` (gated on `hasLoaded` so day-changes don't refetch). Mutations route through `set(day:emoji:)` / `clear(day:)` — optimistic in-memory write + background `upsert` / `delete` against `public.day_marks`. RLS scopes rows to owner; primary key `(user_id, day)` makes set an upsert. Reset via `resetForUserChange()` from `AuthStore`'s sign-out and user-switch handlers, alongside `TimelineStore` and `WeekStripStore`.
...
- **Drag-to-reorder:** long-press any card → drag → drop on a target card.
  ...
  - `KeepCard`'s built-in `.contextMenu` (Pin / Delete; **Edit details** for media notes only) coexists with `.draggable`: tap-and-hold-without-drift opens the menu; tap-and-hold-then-drag begins a reorder. Standard iOS disambiguation, no manual gesture coordination. Edit details opens `MediaDetailsEditSheet` — a `.medium` detent sheet with a multi-line caption field and a `DatePicker(.compact)` for occurred-at. Save reconstructs the `MockNote` with the updated caption + timestamp and routes through `TimelineStore.update`.
...
- **Same row in `MediaNoteEditorScreen`** — visually identical, except its `occurredAt` lives in local `@State` rather than the shared draft store (media notes don't persist drafts). Additionally, when an imported photo or video has capture metadata (`payload.capturedAt`), the picker auto-seeds to that timestamp so the note lands at the moment the media was actually taken — even for older library imports. Falls back to the "selected day + now" default when metadata is missing (screenshots, edited exports). User edits to the picker are sticky across photo replacement.
- **Relative-time hint** under the picker (`MediaNoteEditorScreen` create flow + `MediaDetailsEditSheet` edit flow). Renders in `.DS.caption` / `Color.DS.fg2` when the picker still reflects the underlying media's `capturedAt` AND the gap from now is at least 60 seconds — surfaces "yesterday" / "5 days ago" / "2 weeks ago" / "in 3 hours" via `Date.formatted(.relative(presentation: .named))`. The 60-second floor suppresses the hint for camera captures (where `capturedAt = Date()` at shutter, gap ≈ 0). The moment the user touches the picker, equality breaks and the hint fades out (`.easeInOut(0.2)`); replacing a photo in the create flow while the picker still shows the prior media's timestamp also breaks equality and hides the hint, signalling a mismatch that needs attention.
...
- **Persistence (Phase F.1.2.bgpersist + F.1.2.swatchpersist).** Both background variants round-trip through the `backgrounds` table + (for image) the `note-backgrounds` Supabase Storage bucket. The note's `background_id` FK points at the row. **Image** encode: bytes upload via `MediaStorageProvider.backgrounds`, then a `backgrounds` INSERT with `kind='image'` + `image_url` = bucket-relative path. Decode: signed URL via the backgrounds storage impl → byte download → reconstruct `ImageBackground`. **Swatch** (color) encode: find-or-INSERT pattern keyed by `(user_id, kind='color', swatch_id)` so picking the same swatch on multiple notes reuses one row. Decode: read `swatch_id` from the row, return `.color(swatchId:)`; the iOS palette repository renders the actual color from the design-system swatch JSON. Failures during decode log + return nil so a missing background never blocks loading the rest of the note.
